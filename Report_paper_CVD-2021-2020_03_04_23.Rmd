---
title: 'Impact of Gamma COVID-19 variant on the prognosis of pregnant and postpartum women with cardiovascular disease'
...
```{r, echo=TRUE,message=FALSE,warning =FALSE,error=FALSE}
with(data, ctable(CVD_by_year, hematologic, prop = "r", useNA = "no", chisq = TRUE, OR=TRUE))
```
...
```{r, echo=TRUE,message=FALSE,warning =FALSE,error=FALSE}
ctable(data$CVD_by_year, data$hepatic, chisq=FALSE, prop="r", OR=TRUE, useNA = "no")
```
...
with(df4, ctable(CVD_by_year, intubation, prop="r", OR=TRUE, useNA = "no", chisq = FALSE))
...
with(df4, ctable(CVD_by_year, evolution, prop="r", OR=TRUE, useNA = "no", chisq = FALSE))
